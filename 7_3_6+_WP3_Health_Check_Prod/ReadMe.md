...
graph TD
    ...
    subgraph Pre-Check & Remediation
        D --> E[Run 2-all_XR_python_pre_check.py];
        E --> F[Gather baseline info from outputs];
        F --> G{Issues after baseline?};
        G -- Yes --> H[Perform initial remediation];
        H --> I[Power on device];
        G -- No --> I;
        I --> J[Run 1-all_XR_pre_check_and_comparison.py <br> (re-check optics, interfaces)];
        J --> K{Pre-checks OK?};
        K -- No --> H;
    end
