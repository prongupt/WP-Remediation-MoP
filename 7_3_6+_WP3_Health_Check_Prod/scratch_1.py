...
class TqdmAwareTee:
    ...
    aware of tqdm progress bars to prevent interleaved output.
    """
    def __init__(self, original_console_stream, file_object):
        self.original_console_stream = original_console_stream
        self.file_object = file_object
        self._tqdm_active = False

    def set_tqdm_active(self, active: bool):
        self._tqdm_active = active
    ...
    def write(self, data):
        ...
        # For console, use tqdm.write if active, else direct write
        if self._tqdm_active:
            # tqdm.write handles printing above the bar and managing newlines
            # We use end='' because data might already contain newlines from print/logger
            tqdm.write(data, file=self.original_console_stream, end='')
        else:
            self.original_console_stream.write(data)
...
def main():
    # Store the true original stdout before any redirection
    true_original_stdout = sys.stdout

    console_formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s')

    # Remove any existing StreamHandlers that might write to the console
    for handler in logger.handlers[:]:
        if isinstance(handler, logging.StreamHandler) and handler.stream in (sys.stdout, sys.stderr, true_original_stdout):
            logger.removeHandler(handler)
            handler.close()
    ...
    try:
        ...
        # Create and add the file handler for logging
        file_handler = logging.FileHandler(session_log_path, encoding='utf-8')
        file_handler.setFormatter(console_formatter)
        logger.addHandler(file_handler)

        # Create our TqdmAwareTee instance and redirect sys.stdout to it
        tqdm_aware_stdout = TqdmAwareTee(true_original_stdout, session_log_file_handle)
        sys.stdout = tqdm_aware_stdout

        # Add a StreamHandler to the logger that writes to our TqdmAwareTee (which is now sys.stdout)
        # This ensures all logger messages also go through our custom Tee.
        new_console_handler = logging.StreamHandler(tqdm_aware_stdout)
        ...
        with tqdm(total=total_steps, desc="Overall Device Checks", unit="check", dynamic_ncols=True, leave=True, colour="green") as pbar:
            tqdm_aware_stdout.set_tqdm_active(True) # Inform Tee that tqdm is active
            ...
            tqdm_aware_stdout.set_tqdm_active(False) # Deactivate tqdm-aware writing
        ...
        # Remove the temporary console handler from the logger
        for handler in logger.handlers[:]:
            if isinstance(handler, logging.StreamHandler) and handler.stream == tqdm_aware_stdout:
                logger.removeHandler(handler)
                handler.close()
