...
import platform
import subprocess  # Not explicitly used in the original snippet, kept for consistency
import datetime
import os
import sys
...
from xr_ssh_utils import (
    ...
    Tee,  # Still imported, but its usage is now within setup_dynamic_logging
...
# --- Logger Setup ---
# The root logger is initialized here. Its handlers will be reconfigured by setup_dynamic_logging.
...
# --- Specific Constants for this script ---
...
# --- Exceptions specific to this script's checks ---
...
# --- Utility functions (kept here as they are specific to this script's parsing/logic) ---

def get_chassis_model(shell: paramiko.Channel) -> str:
    ...
    # execute_command_in_shell now returns the output directly
...
def check_fabric_reachability(shell: paramiko.Channel, chassis_model: str = "unknown_chassis"):
...
def check_fabric_link_down_status(shell: paramiko.Channel):
...
def check_npu_link_info(shell: paramiko.Channel):
...
def check_npu_stats_link(shell: paramiko.Channel):
...
def check_npu_driver_status(shell: paramiko.Channel):
...
def check_fabric_plane_stats(shell: paramiko.Channel):
...
def check_asic_errors(shell: paramiko.Channel):
...
def run_show_inventory(shell: paramiko.Channel) -> str:
...
def check_interface_status(shell: paramiko.Channel) -> Tuple[str, str]:
    """
...
def check_and_capture_alarms_and_logs(shell: paramiko.Channel):
    ...
    alarm_output = execute_command_in_shell(shell, alarm_command, "show alarms brief system active (filtered)",
                                            timeout=60, print_real_time_output=False)
    cleaned_alarm_lines = []
...
def check_lc_asic_errors(shell: paramiko.Channel, lc_locations: List[str]):
...
def check_environment_status(shell: paramiko.Channel):
...
def check_ios_xr_version(shell: paramiko.Channel) -> str:
...
def check_hw_module_fpd_status(shell: paramiko.Channel) -> str:
...
def find_earliest_file_as_permanent_baseline(hostname_prefix: str, output_directory: str) -> Optional[str]:
    """
    Finds the earliest _raw_output_*.txt file in the given directory.
    This file serves as the permanent baseline.
    """
    # Changed pattern to match the new raw output filename format
    pattern = re.compile(rf"^{re.escape(hostname_prefix)}_pre_check_python_raw_output_(\d{{8}}_\d{{6}})\.txt$")
...
# ... (rest of the script, including imports, constants, exceptions, and functions) ...

def main():
    original_stdout = sys.stdout

    # The global logger is already set up at the top of the script.
    # We just need to ensure its level is set.
    ...
    # cli_output_file is no longer directly managed here; Tee handles raw output
    session_log_file_handle = None  # Will be set by setup_dynamic_logging
    raw_output_file_handle = None  # Will be set by setup_dynamic_logging
    hostname_dir = None  # Will be set by setup_dynamic_logging
    section_statuses = {}
    hostname = "unknown_host"  # Placeholder, will be updated
    ...
    try:
        # --- PHASE 1: Get Hostname and Setup Dynamic Logging ---
        try:
            hostname = get_sanitized_hostname_from_router(router_ip, username, password, SSH_TIMEOUT_SECONDS)
        except HostnameRetrievalError as e:
            logger.critical(f"Failed to retrieve hostname: {e}. Using IP address for log filename.")
            hostname = router_ip.replace('.', '-')  # Fallback to sanitized IP
            overall_script_failed[0] = True  # Mark as failed due to hostname retrieval issue

        # Setup dynamic logging using the utility function
        ...
        hostname_dir, session_log_file_handle, raw_output_file_handle, timestamp = \
            setup_dynamic_logging(hostname, "pre_check_python", original_stdout)

        # The logger object 'logger' is already global. Its handlers and level are managed by setup_dynamic_logging.
        # No need to re-assign or redeclare it. Just ensure its level is correct if needed.
        logger.setLevel(logging.INFO)

        logger.info(f"--- Cisco IOS-XR Device Status Report & Comparison ---")
        logger.info(f"Sanitized hostname for file paths: {hostname}")
        logger.info(f"Output directory: {hostname_dir}")

        # --- PHASE 2: Establish SSH Connection and Run Checks ---
        ...
        # Set terminal length and width (no cli_output_file arg needed for execute_command_in_shell)
        ...
        # Get chassis model (no cli_output_file arg needed for get_chassis_model)
        chassis_model = get_chassis_model(shell)
        ...
        _run_section_check("Fabric Reachability Check", check_fabric_reachability, section_statuses,
                           overall_script_failed, shell, chassis_model=chassis_model)
        ...
        if ft_locations_from_platform:
            _run_section_check(section_name, check_fan_tray_status, section_statuses, overall_script_failed, shell,
                               ft_locations_from_platform, all_card_inventory_info)
        else:
        ...
        # The filename pattern in find_earliest_file_as_permanent_baseline is updated to match new logging
        permanent_baseline_file_path = find_earliest_file_as_permanent_baseline(hostname, hostname_dir)
        ...
        if permanent_baseline_file_path:
            ...
            print(
                f"The CLI output generated by this run (in {hostname_dir}) will serve as the permanent baseline for next comparisons.")
        ...
        # print_final_summary_table is called here
