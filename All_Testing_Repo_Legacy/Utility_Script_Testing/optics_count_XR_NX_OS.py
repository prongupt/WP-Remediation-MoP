...
# presence and configuration of specific 8800 series line cards and the optics installed within them.
...
# 4.  **Interactive Shell Simulation:** Uses `invoke_shell` to simulate a real user session,
#     bypassing potential authorization issues with non-interactive `exec_command`.
# 5.  **Line Card Identification:** Parses 'show platform' output to accurately
#     identify and list target 8800 line cards that are in an 'IOS XR RUN' state.
# 6.  **Optics Inventory by Slot:** For each identified line card, the script
#     executes 'show inventory location <slot>' to retrieve detailed information about
#     installed optics.
# 7.  **Optics Categorization and Counting:** Parses the 'show inventory location' output
#     to categorize and count optics by their Product ID (PID), providing a clear
#     breakdown of installed transceivers.
# 8.  **Raw Inventory Data Capture:** Automatically saves the full 'show inventory' output
#     for each device to a separate timestamped text file, useful for deeper analysis
#     or record-keeping.
# 9.  **Detailed and Summarized Reporting:** Presents results in a clear, formatted manner:
#     *   Individual device status and any encountered errors.
#     *   Per-line-card details, including total optics and a breakdown by PID.
#     *   Segregated summary tables (IOS-XR vs NX-OS) showing total optics per device, chassis type, slot, and module type.
#     *   Segregated grand total tables showing aggregate optics counts across the fleet.
# 10. **Robust Error Handling:** Includes mechanisms to catch and report SSH connection
