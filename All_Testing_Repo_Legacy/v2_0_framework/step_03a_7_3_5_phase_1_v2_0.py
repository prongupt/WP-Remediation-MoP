...
if __name__ == "__main__":
    ...
    try:
        ...
        if not pre_flight_check(ROUTER_IP, SSH_USERNAME, SSH_PASSWORD, "Part 3a - Phase 1"):
        ...
        # FIXED: Add the complete file setup code that was missing
        try:
        ...
        workflow_manager.dashboard.start_phase("Phase 1 - Steps a-e")

        # ENHANCED Step a: Dummy yes with lambda wrapper
        ...
        try:
            ...
            workflow_manager.dashboard.update_phase_progress("Phase 1", 1, 5, "Step a Complete")
        ...
        # ENHANCED Step b: Monitor dataplane with lambda wrapper
        logging.info(f"\n{'#' * 70}\n### Step b: First Dataplane Monitor ###\n{'#' * 70}\n")
        try:
            smart_retry_with_context(
                lambda: run_dataplane_monitor_phase(ROUTER_IP, SSH_USERNAME, SSH_PASSWORD, "FIRST", SSH_TIMEOUT_SECONDS,
                                                    DATAPLANE_MONITOR_TIMEOUT_SECONDS),
                max_retries=2,
                context="Step b - Dataplane Monitor"
            )
            results_summary["Step b"] = "First Dataplane Monitor: Success"
            workflow_manager.dashboard.update_phase_progress("Phase 1", 2, 5, "Step b Complete")
        ...
        # ENHANCED Step c: Wait time with progress tracking
        logging.info(f"\n{'#' * 70}\n### Step c: {WAIT_TIME_MINUTES}-minute Wait Time ###\n{'#' * 70}\n")
        try:
            workflow_manager.dashboard.update_phase_progress("Phase 1", 3, 5, f"Step c - {WAIT_TIME_MINUTES}min Wait")
            colorful_countdown_timer(WAIT_TIME_MINUTES * 60)
            results_summary["Step c"] = f"{WAIT_TIME_MINUTES}-minute Wait: Success"
            workflow_manager.dashboard.update_phase_progress("Phase 1", 3, 5, "Step c Complete")
        ...
        # ENHANCED Step d: Dummy no with lambda wrapper
        ...
        try:
            ...
            workflow_manager.dashboard.update_phase_progress("Phase 1", 4, 5, "Step d Complete")
            ...
            if "Degraded links found" in str(e):
                ...
                logging.critical(f"\033[1;91m✗ Dummy no phase failed: Degraded links found\033[0m")

                # ENHANCED: Specific degraded links recovery guidance
                print("🚨 CRITICAL: Degraded Links Detected in Phase 1")
                print("📋 Required Actions Before Proceeding:")
                print("   🔍 1. Review link degradation analysis above")
                print("   🔧 2. Investigate and fix physical layer issues")
                print("   🔄 3. Consider LC/FC reseating or replacement")
                print("   ⚠️  4. Do NOT proceed to Phase 2 until resolved")
            else:
                suggest_recovery_actions(type(e), "Step d", "Dummy no execution")

            script_aborted = True
            # DON'T re-raise - let it flow to finally block
        except Exception as e:
            results_summary["Step d"] = f"Dummy No: Failed - {e}"
            script_aborted = True
            # DON'T re-raise - let it flow to finally block

        # Step e: Manual intervention (only if Step d succeeded)
        if not script_aborted:
            logging.info(f"\n{'#' * 70}\n### Step e: MANUAL INTERVENTION REQUIRED ###\n{'#' * 70}\n")
            try:
                workflow_manager.dashboard.update_phase_progress("Phase 1", 5, 5, "Step e - Manual Reload")
                logging.critical("Please perform the two reloads now.")

                # ENHANCED: Detailed manual intervention guidance
                print("🔄 RELOAD PROCEDURE GUIDANCE:")
                print("   1. Perform first reload: hard reload")
                print("   2. Wait for 30 minutes complete system startup")
                print("   3. Perform second reload: hard reload")
                print("   4. Wait for 30 minutes complete system startup")
                print("   5. Verify all cards are operational")

                results_summary["Step e"] = "Manual Reload Step: Instructed User"
                workflow_manager.dashboard.update_phase_progress("Phase 1", 5, 5, "Step e Complete")
        ...
        suggest_recovery_actions(type(e), "Part 3a", str(e))
        script_aborted = True
        ...
        # ENHANCED: Simplified workflow state management
        if workflow_manager:
            errors = [v for v in results_summary.values() if "Failed" in v]
            workflow_manager.state.save_phase_completion("step_03a", results_summary, errors)
            workflow_manager.dashboard.complete_phase("Phase 1", success=not script_aborted, errors=errors)
        ...
        # ENHANCED: Simple next steps guidance
        if workflow_manager and not script_aborted:
            print("\n🚀 NEXT STEPS:")
            print("   1. Complete the manual reloads as instructed")
            print("   2. Verify all system cards are operational")
            print("   3. Execute Part 3b when ready")