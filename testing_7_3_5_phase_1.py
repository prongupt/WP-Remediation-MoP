...
from common_utils import (
    ...
    format_and_print_error_report, run_dataplane_monitor_phase, execute_script_phase, # <--- ADDED execute_script_phase
    print_final_summary,
...
if __name__ == "__main__":
    ...
    try:
        ...
        # e) Start countdown timer for 65 minutes
        logging.info(f"\n{'#' * 70}")
        logging.info(f"### Step e: {COUNTDOWN_DURATION_MINUTES}-minute Countdown Timer ###")
        try:
            colorful_countdown_timer(COUNTDOWN_DURATION_MINUTES * 60)
            results_summary["Step e"] = f"{COUNTDOWN_DURATION_MINUTES}-minute Countdown: Success"
            logging.info(f"{COUNTDOWN_DURATION_MINUTES}-minute countdown completed.")
        except Exception as e:
            results_summary["Step e"] = f"{COUNTDOWN_DURATION_MINUTES}-minute Countdown: Failed - {e}"
            logging.critical(f"{COUNTDOWN_DURATION_MINUTES}-minute countdown failed: {e}")
            script_aborted = True
            raise
